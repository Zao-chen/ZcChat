    ...
    , ui(new Ui::setting_actor_tachie_listchild)
{
    ...
    QSettings * settings2 = new QSettings(QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation) + "/ZcChat/Setting.ini", QSettings::IniFormat);
    // 构造 anim.ini 的完整路径
    QString iniPath2 = QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation)
                      + "/ZcChat/characters/"
                      + settings->value("actor/name").toString()
                      + "/ptc.ini";
    // 创建 QSettings 对象读取 INI 文件（注意 format 参数是 IniFormat）
    QSettings animSettings2(iniPath2, QSettings::IniFormat);
    // 读取 "name/animation" 项
    ui->comboBox_ptc->setCurrentIndex(animSettings2.value(name.split(".")[0]+"/particle").toInt());
    has_init = true;
}
